...
BOOL CNatThread::Run()
{
	...
	{
		uint32 cid=0;
		if(theApp.serverconnect) cid=theApp.serverconnect->GetClientID();
		...
		if(cid==0 || !::IsLowID(cid))
		{
			if(!cid)
			{
				...
				{
					if(time(NULL) - tmCheckKad < 10)
					{
						Sleep(2000);
						continue;
					}
				}
				else if(!Kademlia::CKademlia::IsConnected() && theApp.clientlist && theApp.clientlist->GetClientCount()==0 )
				{
					Sleep(1000);
					continue;
				}
...
void CNatThread::SendConnBuffer(bool /*bByBuddy*/)
{
	...
	//	CUpDownClient * client= theApp.clientlist->FindClientByUserHash(uh.userhash);
...
bool CNatThread::SwitchNextStrategy(const USERHASH & UsrHash)
{
	...
	if(strategy)
	{
		...
		if(! pNext)
		{
			CUpDownClient * client=theApp.clientlist->FindClientByUserHash(strategy->GetUserHash());
			if(client && client->socket)
			{
...
void CNatThread::OnSync(DWORD ip, WORD port, const uchar * pData, int nDataSize)
{
	...
	if(! usock)
	{
		...
		usock->m_dwConnAskNumber = ConnAck; //0;
...
int CUnconnSocket::SendPingPacket()
{
	...
	T_TRACE("send ping packet for %02X%02X%02X%02X. [%s:%04x] ConnAck=x%08x\n", m_ClientId[0],m_ClientId[1],m_ClientId[2],m_ClientId[3],ipstrA(m_dwClientIp),m_wClientPort,
		m_dwConnAskNumber);
...
{
	if(!theApp.clientudp) return;
	
	_AddDebugLogLine(false, _T("CNatThread::RegisterMe"));
	
	if(!m_bRegister)
		AddLogLine(false, _T("Registering to NAT traversal server\n"));
