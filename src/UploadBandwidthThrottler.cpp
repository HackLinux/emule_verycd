...
#include "ListenSocket.h"// VC-kernel[2007-01-18]:
#include "updownclient.h"// VC-kernel[2007-01-18]:
...
uint32 UploadBandwidthThrottler::GetSlotLimit(uint32 currentUpSpeed) {
	...
	if( currentUpSpeed > 30*1024 )
		upPerClient += currentUpSpeed/58;
	...
	else if (currentUpSpeed > 12*1024)
		nMaxSlots = MIN_UP_CLIENTS_ALLOWED * 2;
	else if (currentUpSpeed > 6*1024)
		nMaxSlots = MIN_UP_CLIENTS_ALLOWED * 3 / 2;
	else
...
UINT UploadBandwidthThrottler::RunInternal() {
	...
	/*DWORD nUploadStartTime = 0;*/					//Deleted by thilon on 2006.05.18
    /*uint32 numberOfConsecutiveUpChanges = 0;*/	//Deleted by thilon on 2006.05.18
    /*uint32 numberOfConsecutiveDownChanges = 0;*/	//Deleted by thilon on 2006.05.18
    uint32 changesCount = 0;
    uint32 loopsCount = 0;

    /*bool estimateChangedLog = false;*/			//Deleted by thilon on 2006.05.18
	...
	while(doRun) {
		...
		/*Deleted by thilon on 2006.05.18 -以下一段被注释掉，该段可能会导致没有上传用户
		Note:所有使用bUploadUnlimited变量的地方会一起被注释掉，恢复时请一同恢复*/
...
#define TIME_BETWEEN_UPLOAD_LOOPS 1
        ...
        if(timeSinceLastLoop < sleepTime) {
			if( sleepTime-timeSinceLastLoop < 100 )
        ...
        if(bytesToSpend >= 1 || allowedDataRate == 0) {
		    ...
		    for(uint32 slotCounter = 0; slotCounter < (uint32)m_StandardOrder_list.GetSize(); slotCounter++) {
				...
				//  if busy, don't send data while OnSend is called
			    if(socket != NULL) {
				if( !socket->IsBusy()) {
				    if(thisLoopTick-socket->GetLastCalledSend() > SEC2MS(1)) {
					    ...
					    if(neededBytes > 0) {
                            // VC-kernel[2007-01-18]:change start
						    SocketSentBytes socketSentBytes = {true,0,0};
                            if(socket->IsLan())
                                socket->SendFileAndControlData(neededBytes, minFragSize);
                            else
                                socketSentBytes = socket->SendFileAndControlData(neededBytes, minFragSize);
                            // VC-kernel[2007-01-18]:change end
                            uint32 lastSpentBytes = socketSentBytes.sentBytesControlPackets + socketSentBytes.sentBytesStandardPackets;
						    spentBytes += lastSpentBytes;
						    spentOverhead += socketSentBytes.sentBytesControlPackets;
            ...
            for(uint32 maxCounter = 0; maxCounter < min(maxSlot, (uint32)m_StandardOrder_list.GetSize()) && bytesToSpend > 0 && spentBytes < (uint64)bytesToSpend; maxCounter++) {
				...
				if( !socket->IsBusy()) {
					...
					SocketSentBytes socketSentBytes = {true,0,0};
                    if(socket->IsLan())
                        socket->SendFileAndControlData((UINT)min(doubleSendSize, bytesToSpend-spentBytes), doubleSendSize);// VC-kernel[2007-01-18]:
                    else
                        socketSentBytes = socket->SendFileAndControlData((UINT)min(doubleSendSize, bytesToSpend-spentBytes), doubleSendSize);
                    // VC-kernel[2007-01-18]:change end
					uint32 lastSpentBytes = socketSentBytes.sentBytesControlPackets + socketSentBytes.sentBytesStandardPackets;
			...
			for(uint32 slotCounter = 0; slotCounter < (uint32)m_StandardOrder_list.GetSize() && bytesToSpend > 0 && spentBytes < (uint64)bytesToSpend; slotCounter++) {
				...
				if(!socket->IsBusy()) {
                    ...
                    if(socket->IsLan())
                        ...
                        socketSentBytes = socket->SendFileAndControlData(bytesToSpendTemp, doubleSendSize);
                    // VC-kernel[2007-01-18]:change end
                    uint32 lastSpentBytes = socketSentBytes.sentBytesControlPackets + socketSentBytes.sentBytesStandardPackets;
