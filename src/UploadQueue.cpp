...
bool CUploadQueue::AcceptNewClient(uint32 curUploadSlots){
	...
	//Changed by thilon on 2006.08.16, 固定上传线程
...
bool CUploadQueue::ForceNewClient(bool allowEmptyWaitingQueue) {
	...
	if (::GetTickCount() - m_nLastStartUpload < 1000 && datarate < 1024 * 100 )
		return false;
	...
	if( MaxSpeed > 30 || MaxSpeed == UNLIMITED)
		upPerClient += datarate/58;
...
void CUploadQueue::AddClientToQueue(CUpDownClient* client, bool bIgnoreTimelimit)
{
    ...
    //if(IsLANIP(client->GetIP())){
    //    client->SetSourceFrom(SF_LAN);
        //client->GetFileUploadSocket()->IsLan = true;
    //}
	...
	for (pos1 = waitinglist.GetHeadPosition();( pos2 = pos1 ) != NULL;)
	{
		...
		else if ( client->GetIP()!=0 && client->GetIP()==cur_client->GetIP() )
		{
	...
	int iMaxSameIP = client->HasLowID() ? 9 : 3; // VC-Huby[2006-12-22]:修改SameIP限制策略

	if (cSameIP >= iMaxSameIP && !client->HasLowID() ) //Same HighId IP
	{
	...
	else if ( client->GetIP()!=0 && theApp.clientlist->GetClientsFromIP(client->GetIP()) >= iMaxSameIP )
	{
		if (thePrefs.GetVerbose())
...
bool CUploadQueue::CheckForTimeOver(CUpDownClient* client){
	...
	if (!thePrefs.TransferFullChunks()){
		...
		if( client->GetQueueSessionPayloadUp() > SESSIONMAXTRANS ){
			if( client->GetUpStartTimeDelay() > MIN2MS(10) )
			{
			if (thePrefs.GetLogUlDlEvents())
...
VOID CALLBACK CUploadQueue::UploadTimer(HWND /*hwnd*/, UINT /*uMsg*/, UINT_PTR /*idEvent*/, DWORD /*dwTime*/)
{
	...
	try
	{
		...
		if(Kademlia::CKademlia::LostConnection())
		{
		...
		if (counter >= 10){
...
#ifdef _DEBUG
			theApp.m_pMemoryPool->ProcessLog();
#endif			
#endif
			...
			//ADDED by fengwen on 2007/04/11	<begin> :
			theStatForServer.RecordCurrentRate((float)(theApp.uploadqueue->GetDatarate())/1024,(float)(theApp.downloadqueue->GetDatarate())/1024);
			//ADDED by fengwen on 2007/04/11	<end> :
...
#ifdef _SUPPORT_MEMPOOL
				theApp.m_pMemoryPool->CheckToRelease(); 
#endif
				theApp.serverconnect->CheckStatTimeOut(); // VC-SearchDream[2007-06-14]: Report to Stat Server
			}
