...
#define	NUM_BUTTON_BITMAPS	15 //Changed by thilon On 2006.08.01, 14 -> 15
...
void CMuleToolbarCtrl::Init(void)
{
	...
	//added by thilon on 2006.08.01, ä¯ÀÀÆ÷°´Å¥ 
	lLen2 = _tcslen(GetResString(IDS_EM_WEBBROWSER)) + 1;
	memcpy(cButtonStrings+lLen, GetResString(IDS_EM_WEBBROWSER), lLen2*sizeof(TCHAR));
	lLen += lLen2;
	++m_buttoncount;
#endif
	// terminate
	...
	for (int j = GetButtonCount()-1; j >= 0; j--)
	{
		TBBUTTON Button;
		GetButton(j, &Button);

		if (Button.idCommand == TBBTN_WEBBROWSER && !thePrefs.m_bShowBroswer)
		{
			DeleteButton(j);
		}

		if (Button.idCommand == 0 && !thePrefs.m_bShowBroswer)
		{
			DeleteButton(j);
		}
	}
...
void CMuleToolbarCtrl::SetAllButtonsStrings()
{
	static const int TBStringIDs[] =
	{
		...
		IDS_TOOLS,
		IDS_EM_HELP,
		IDS_EM_WEBBROWSER		//added by thilon on 2006.08.01
	};
	TBBUTTONINFO tbi;
...
void CMuleToolbarCtrl::ChangeToolbarBitmap(const CString& path, bool bRefresh)
{
	...
	if (!bResult)
	{
		...
		ImageList.Add(CTempIconLoader(_T("WEBBROWSER"), m_sizBtnBmp.cx, m_sizBtnBmp.cy)); // Added By thilon on 2006.08.01
...
BOOL CMuleToolbarCtrl::OnCommand(WPARAM wParam, LPARAM /*lParam*/)
{
	switch (wParam)
	{
		case MP_SELECTTOOLBARBITMAPDIR:{
			...
			_sntprintf(buffer,ARRSIZE(buffer),_T("%s"), thePrefs.GetMuleDirectory(EMULE_TOOLBARDIR));
			if(SelectDir(m_hWnd, buffer, GetResString(IDS_SELECTTOOLBARBITMAPDIR)))
			{
				thePrefs.SetMuleDirectory(EMULE_TOOLBARDIR, buffer);//Changed by thilon on 2007.05.28//thePrefs.SetToolbarBitmapFolderSettings(buffer);
			}
			break;
		}
		...
		case MP_SELECT_SKIN_DIR:{
			TCHAR buffer[MAX_PATH];
			_sntprintf(buffer,ARRSIZE(buffer),_T("%s"), thePrefs.GetMuleDirectory(EMULE_SKINDIR, false));
			if(SelectDir(m_hWnd, buffer, GetResString(IDS_SELSKINPROFILEDIR)))
			{
				thePrefs.SetMuleDirectory(EMULE_SKINDIR, buffer);//Changed by thilon on 2007.05.28//thePrefs.SetSkinProfileDir(buffer);
			}
			break;
		}
...
void CMuleToolbarCtrl::Refresh()
{
	...
	else
	{
		...
		CWnd* wnds[] =
		{
			...
			theApp.emuledlg->webbrowser // Added by thilon on 2006.08.01
		};
...
void CMuleToolbarCtrl::OnTbnEndAdjust(NMHDR*, LRESULT* pResult)
{
	for (int i = 0; i < GetButtonCount();i++)
	{
		...
		if (GetButton(i, &buttoninfo))
		{
			if (buttoninfo.idCommand == TBBTN_WEBBROWSER)
			{
				thePrefs.m_bShowBroswer = 1;
				break;
			}
