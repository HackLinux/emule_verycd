...
const CCheckConflict::CONFILCT_MOD_ENTRY	CCheckConflict::s_conflictModuleList[] = {
	...
	_T("Cjktl32.dll"), _T("金山词霸"), 0, CCheckConflict::SimplyPrompt,
...
BOOL CCheckConflict::SimplyPrompt(int iModuleIndex, HMODULE hMod)
{
	...
	MessageBox(NULL, szOutput, _T("eMule与其他软件冲突"), MB_OK);
...
{
	//*Warning* 修改显示方式时，注重下面szOutput申请的内存是否足够容纳要写入的字符。
	LPCTSTR lpcszOutputFormat = _T("[%s]处理出错，导致eMule退出。\r\n");
	...
	MessageBox(NULL, szOutput, _T("其他软件出错导致eMule退出"), MB_OK);
