...
BOOL CTransferWnd::OnCommand(WPARAM wParam, LPARAM /*lParam*/)
{
	...
	{ 
		...
		case MP_CAT_EDIT: {
			...
			CString csName;
			csName.Format(_T("%s"), thePrefs.GetCategory(rightclickindex)->strTitle );
		...
		case MP_PLAY:
			theApp.downloadqueue->SetCatStatus(rightclickindex, MP_PLAY); // VC-SearchDream[2007-05-16]: for see movie while downloading
			break;
#endif 
		case MP_RESUMENEXT:
...
void CTransferWnd::UpdateCatTabTitles(bool force)
{
	...
	//MODIFIED by VC-fengwen on 2007/10/10 <begin> : thePrefs.GetCategory(i) 有可能返回空。
		//EditCatTabLabel(i,/*(i==0)? GetCatTitle( thePrefs.GetCategory(0)->filter ):*/thePrefs.GetCategory(i)->strTitle);
	{
		...
		if (NULL != pCatStruct)
			EditCatTabLabel(i, pCatStruct->strTitle);
	}
	//MODIFIED by VC-fengwen on 2007/10/10 <end> : thePrefs.GetCategory(i) 有可能返回空。
}
