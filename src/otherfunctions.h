...
inline const TCHAR* s_addr_port(TCHAR* buf,uint32 t, int port)
{
	if(buf==NULL) return _T("");
	buf[0]=0;
	unsigned char* temp = (unsigned char*)&t;
	_stprintf(buf,_T("%d.%d.%d.%d|%d"),(int)temp[0],(int)temp[1],(int)temp[2],(int)temp[3],port);
	return buf;
}
...
inline uint32 ExpoBackoff(uint32 uBase, uint32 uCount)
{
	if (uCount > 31)
		uCount = 31;
