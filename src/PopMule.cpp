...
#ifdef _DEBUG
			StartPlayer(m_pPartFile->GetFileHash(), m_pPartFile->GetFileName(), m_pPartFile->GetFileSize().operator uint32(), strFileExt);
#else
			StartPlayer(m_pPartFile->GetFileHash(), m_pPartFile->GetFileName(), (DWORD)m_pPartFile->GetFileSize(), strFileExt);
#endif
		}
	}
