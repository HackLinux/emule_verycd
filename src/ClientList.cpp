...
void CClientList::DeleteAll()
{
	try
	{
		...
		// VC-Huby[2006-12-19]: free badbuddy object
		for (pos1 = m_BadBuddyList.GetHeadPosition();( pos2 = pos1 ) != NULL;)
		{
			delete m_BadBuddyList.GetNext(pos1);;
		}
		m_BadBuddyList.RemoveAll();
	}
	catch (...)
	{		
	}	
...
#ifdef _DEBUG_KAD_
					TCHAR temp[30];
					Debug( _T("TryToConnect-Buddy,BuddyInfo:%s,\r\n"),s_addr_port(temp,cur_client->GetConnectIP(),cur_client->GetKadPort() ) );
#endif
				}
...
#ifdef _DEBUG_KAD_
					TCHAR temp[30];
					Debug( _T("找到 Buddy,BuddyInfo:%s, kad port=%d\r\n"),s_addr_port(temp,m_pBuddy->GetIP(),m_pBuddy->GetUserPort() ) , 
						m_pBuddy->GetKadPort());
#endif
				}
...
#ifdef _DEBUG_KAD_
			if( Connecting==m_nBuddyStatus )
				Debug( _T("丢失 (Connecting)Buddy.\r\n") );
			else
				Debug( _T("丢失 (Connected)Buddy.\r\n") );
#endif
			if( m_pBuddy )
			{
				AddBadBuddy( m_pBuddy->GetIP(),m_pBuddy->GetUserPort(),m_pBuddy->GetUDPPort(),m_pBuddy->GetUserHash() );
			}
...
void CClientList::RequestBuddy(Kademlia::CContact* contact, uint8 byConnectOptions)
{
	...
	if( IsBadBuddy( ntohl(contact->GetIPAddress()),contact->GetTCPPort() ) )
		return;
...
void CClientList::ProcessConnectingClientsList(){
	...
	for (pos1 = m_liConnectingClients.GetHeadPosition();( pos2 = pos1 ) != NULL;){
		...
		uint32 uConnectingTimeOut = SEC2MS(45);
		if( cc.pClient->socket && cc.pClient->socket->m_bUseNat )
		{
			uConnectingTimeOut = SEC2MS(160);
		}
		if (cc.dwInserted + uConnectingTimeOut < cur_tick)
		{
...
///////// VC-Huby[2006-12-19]: For Bad Buddy BlackList ////////////////////////////////////////////////////

bool CClientList::IsBadBuddy( DWORD dwIP,uint16 iTcpPort )
{
	POSITION pos1,pos2;

	for (pos1 = m_BadBuddyList.GetHeadPosition(); (pos2 = pos1) != NULL; )
	{
		m_BadBuddyList.GetNext(pos1);
		CBadBuddy* pBadBuddy =	m_BadBuddyList.GetAt(pos2);
		ASSERT( pBadBuddy!=NULL );
		
		if( NULL!=pBadBuddy && dwIP==pBadBuddy->m_dwIP  && iTcpPort==pBadBuddy->m_iPort
			&& (::GetTickCount()-pBadBuddy->m_iLastConnect)<8*60*1000 ) //8分钟以内曾经出过错的Buddy
			return true;
	}
...
CBadBuddy* CClientList::AddBadBuddy( DWORD dwIP,uint16 iTcpPort,uint16 iUdpPort,const uchar* pUserHash )
{
	POSITION pos1,pos2;
	
	CBadBuddy* pNewBadBuddy=NULL;
	...
	for (pos1 = m_BadBuddyList.GetHeadPosition(); (pos2 = pos1) != NULL; )
	{
		...
		if( NULL!=pBadBuddy && dwIP==pBadBuddy->m_dwIP  && iTcpPort==pBadBuddy->m_iPort )
		{
			pNewBadBuddy = pBadBuddy; //Have Exist;
		}
	}

	if( pNewBadBuddy==NULL )
	{
		pNewBadBuddy = new CBadBuddy( dwIP,iTcpPort,iUdpPort );
		pNewBadBuddy->SetUserHash( pUserHash );
		m_BadBuddyList.AddTail( pNewBadBuddy );
	}
	
	pNewBadBuddy->m_iLastConnect = GetTickCount();
	pNewBadBuddy->m_iLostTimes++;

	return pNewBadBuddy;
}

/**
 *	<BR>功能说明：从Buddy黑名单列表中再找出一个候选者作为buddy
 *	<BR>可访问性：
 *	<BR>注    释：
 *	<BR>作    者：Huby
 *	<BR>日    期：2006-12-19
 ...
 *	@return 无
 */
CBadBuddy* CClientList::GetCandidateBuddyFromBlackList( )
{
	POSITION pos1,pos2;

	CBadBuddy* pCandidateBuddy = NULL;
	...
	for (pos1 = m_BadBuddyList.GetHeadPosition(); (pos2 = pos1) != NULL; )
	{
		...
		if( pCandidateBuddy==NULL )
			pCandidateBuddy = pBadBuddy;
		else if( pBadBuddy->m_iLastConnect<pCandidateBuddy->m_iLastConnect )
			pCandidateBuddy = pBadBuddy;
		else if( pBadBuddy->m_iLostTimes<pCandidateBuddy->m_iLostTimes )
			pCandidateBuddy = pBadBuddy;
	}

	return pCandidateBuddy;	
}