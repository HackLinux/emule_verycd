...
public:
	...
	void AddToResolve(const uchar* fileid, LPCSTR pszHostname, uint16 port, LPCTSTR pszURL = NULL, LPCTSTR lpszRefer = NULL);
...
private:
	struct Hostname_Entry {
		...
		uint16 port;
		CString strURL;
		CString strRefer;	//ADDED by fengwen on 2006/09/29 : 保存Url的Refer。
	};
