			...
			switch (wParam)
			{
				...
				case MPG_DELETE: // keyboard del will continue to remove completed files from the screen while cancel will now also be available for complete files
				{
					if (selectedCount > 0)
					{
						...
						if ((removecompl && !validdelete) || (validdelete && AfxMessageBox(quest + GetDisplayLinks(fileList), MB_DEFBUTTON2 | MB_ICONQUESTION | MB_YESNO) == IDYES))
						{
				...
				case MP_PLAY: // VC-SearchDream[2007-05-16]: for see movie while downloading
					{
						...
						{
							CPartFile *partfile = selectedList.GetHead();
							partfile->SetSeeOnDownloading(true);
						}
					}
					break;
#endif
...
#ifdef _PLAY_WHILE_DOWNLOADING
	m_FileMenu.AppendMenu(MF_STRING, MP_PLAY, GetResString(IDS_DL_PLAY), _T("RESUME")); // VC-SearchDream[2007-05-16]: for see movie while downloading
#endif
