...
bool CUrlClient::SendHttpBlockRequests()
{
	...
	//ADDED by fengwen on 2006/09/12 <begin> : 	
	if ( 0==m_strHostA.CompareNoCase("verycd.com") || m_strHostA.MakeLower().Find(".verycd.com")>=0 )
		strHttpRequest.AppendFormat("User-Agent: VeryCD emule v%s\r\n",CStringA(theApp.m_strCurVersionLong));	//浏览器客户端标示，告诉我们自己的服务器这是eMule，另外可以附上版本号，用户操作系统等信息
	else
		strHttpRequest.AppendFormat("User-Agent: Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.0; .NET CLR 1.1.4322)\r\n");	//浏览器客户端标示，伪装成IE浏览器

	CStringA	straRefer;
	if (m_strRefer.IsEmpty())
		straRefer = "http://" + m_strHostA + "/";
	else
		straRefer = m_strRefer;
		
	strHttpRequest.AppendFormat("Referer: %s\r\n", straRefer);	//文件的来源，简单来说，就是用户从哪个页面上点到这个文件的，emule获取不到实际信息，只能伪造。
	//ADDED by fengwen on 2006/09/12 <end> : 	
...
bool CUrlClient::ProcessHttpDownResponse(const CStringAArray& astrHeaders)
{
	...
	//ADDED by fengwen on 2006/09/06 <begin> : 请求url出错后重试。
	if (!bExpectData
		&& !bValidContentRange
	...
	//ADDED by fengwen on 2006/09/06 <end> : 请求url出错后重试。
