...
int CTraverseBySvr::SendPingPacket()
{
	...
	T_TRACE("send ping packet to [%s:%04x] for %02X%02X%02X%02X. ConnAsk=x%08x\n",ipstrA(m_dwClientIp),m_wClientPort, m_UserHash[0],m_UserHash[1],m_UserHash[2],m_UserHash[3],
		m_dwConnAskNumber);
