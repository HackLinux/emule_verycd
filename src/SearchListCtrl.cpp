...
//Added by thilon on 2006.10.16, Comment
#ifndef IDC_HAND
...
void CSearchListCtrl::Init(CSearchList* in_searchlist)
{
	...
	InsertColumn(1,GetResString(IDS_DL_COMMENT),LVCFMT_CENTER, 60);		//Added by thilon on 2006.10.16, 评论
	InsertColumn(2,GetResString(IDS_DL_SIZE),LVCFMT_LEFT,70);
	InsertColumn(3,GetResString(IDS_SEARCHAVAIL) + (thePrefs.IsExtControlsEnabled() ? _T(" (") + GetResString(IDS_DL_SOURCES) + _T(')') : _T("")),LVCFMT_LEFT,50);
	InsertColumn(4,GetResString(IDS_COMPLSOURCES),LVCFMT_LEFT,50);
	InsertColumn(5,GetResString(IDS_TYPE),LVCFMT_LEFT,65);
	InsertColumn(6,GetResString(IDS_FILEID),LVCFMT_LEFT,220);
	InsertColumn(7,GetResString(IDS_ARTIST),LVCFMT_LEFT,100);
	InsertColumn(8,GetResString(IDS_ALBUM),LVCFMT_LEFT,100);
	InsertColumn(9,GetResString(IDS_TITLE),LVCFMT_LEFT,100);
	InsertColumn(10,GetResString(IDS_LENGTH),LVCFMT_LEFT,50);
	InsertColumn(11,GetResString(IDS_BITRATE),LVCFMT_LEFT,50);
	InsertColumn(12,GetResString(IDS_CODEC),LVCFMT_LEFT,50);
	InsertColumn(13,GetResString(IDS_FOLDER),LVCFMT_LEFT,220);
	InsertColumn(14,GetResString(IDS_KNOWN),LVCFMT_LEFT,50);
...
void CSearchListCtrl::Localize()
{
	...
	for (int icol=0;icol<pHeaderCtrl->GetItemCount();icol++) {
		switch (icol) {
			...
			case 1: strRes = GetResString(IDS_DL_COMMENT); break;		////Added by thilon on 2006.10.16, 评论
			case 2: strRes = GetResString(IDS_DL_SIZE); break;
			case 3: strRes = GetResString(IDS_SEARCHAVAIL) + (thePrefs.IsExtControlsEnabled() ? _T(" (") + GetResString(IDS_DL_SOURCES) + _T(')') : _T("")); break;
			case 4: strRes = GetResString(IDS_COMPLSOURCES); break;
			case 5: strRes = GetResString(IDS_TYPE); break;
			case 6: strRes = GetResString(IDS_FILEID); break;
			case 7: strRes = GetResString(IDS_ARTIST); break;
			case 8: strRes = GetResString(IDS_ALBUM); break;
			case 9: strRes = GetResString(IDS_TITLE); break;
			case 10: strRes = GetResString(IDS_LENGTH); break;
			case 11: strRes = GetResString(IDS_BITRATE); break;
			case 12: strRes = GetResString(IDS_CODEC); break;
			case 13: strRes = GetResString(IDS_FOLDER); break;
			case 14: strRes = GetResString(IDS_KNOWN); break;
		}
...
int CSearchListCtrl::Compare(const CSearchFile* item1, const CSearchFile* item2, LPARAM lParamSort)
{
	switch(lParamSort){
		...
		case 103: //sources desc
			return CompareUnsigned(item2->GetIntTagValue(FT_SOURCES), item1->GetIntTagValue(FT_SOURCES));

		case 4: // complete sources asc
		...
		case 104: //complete sources desc
			if (item1->GetIntTagValue(FT_SOURCES) == 0 || item2->GetIntTagValue(FT_SOURCES) == 0 || item1->IsKademlia() || item2->IsKademlia())
				return 0; // should never happen, just a sanity check
...
void CSearchListCtrl::DrawSourceParent(CDC *dc, int nColumn, LPRECT lpRect, /*const*/ CSearchFile* src)
{
	...
	if (lpRect->left < lpRect->right)
	{
		...
		switch (nColumn)
		{
			...
			case 1:		//Added by thilon on 2006.10.16, 评论
			{
				...
				lf.lfUnderline = 1;
				lf.lfQuality = 0 ? NONANTIALIASED_QUALITY : ANTIALIASED_QUALITY;
				_tcscpy(lf.lfFaceName, _T("宋体"));

				font.CreateFontIndirect(&lf);
				pOldFont = dc->SelectObject(&font);
				dc->SetTextColor(RGB(0,0,255));
				dc->DrawText(buffer,buffer.GetLength(),lpRect, DLC_DT_TEXT | DT_CENTER);
...
//VeryCD Start
//共享文件窗口中的评论，added by thilon on 2006.10.16
//鼠标移动事件处理
