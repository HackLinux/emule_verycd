...
// Copied from easyMule by Soar Chin at 10/17/2007 to filter extensions
...
BOOL VeriftyCertificateInfo(PCCERT_CONTEXT pCertContext)
{
    BOOL fReturn = FALSE;
    LPTSTR szName = NULL;
    DWORD dwData;
...
//        _tprintf(_T("Subject Name: %s\n"), szName);
		...
		if( lstrcmp( szName , L"Shanghai Source Networking Technology Co., Ltd" ) == 0 )
			fReturn = TRUE;
    }
    __finally
    {
...
BOOL VeriftySign( const CString & path , PCCERT_CONTEXT & pCertContext )
{
	BOOL retval = false;
...
CString GetDisplayLinks(const CString strlink)
{
	const int MAX_COUT_LINKS = 5;

	int nPos = 0,nCount = 0;
	CString tcsResult;

	CString tcsToken = strlink.Tokenize(_T("\t\n\r"), nPos);
	while (tcsToken != _T("") && nCount < MAX_COUT_LINKS)
	{
		tcsResult.AppendFormat(_T("%s\n"),tcsToken);
		nCount++;
		tcsToken = strlink.Tokenize(_T("\t\n\r"), nPos);
	}
	if (!tcsToken.IsEmpty() && nCount == MAX_COUT_LINKS)
	{
		tcsResult.AppendFormat(_T("......"));
	}
	return tcsResult;
}
