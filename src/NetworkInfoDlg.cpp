...
//#ifdef _DEBUG_KAD_ // VC-Huby[2006-12-29]: shouw low2low link url
		...
		if( theApp.clientlist->GetBuddyStatus()==Connected && theApp.clientlist->GetBuddy() )		
		{			
			CString KadID;
			Kademlia::CKademlia::GetPrefs()->GetKadID().Xor(Kademlia::CUInt128(true)).ToHexString(&KadID);
			rCtrl << KadID << _T("|");
			TCHAR tempBuddyInfo[30];
			rCtrl << s_addr_port( tempBuddyInfo,theApp.clientlist->GetBuddy()->GetConnectIP(),theApp.clientlist->GetBuddy()->GetUDPPort() );
			rCtrl << _T("|");
		}
		else
		{
