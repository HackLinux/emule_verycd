...
float CClientCredits::GetScoreRatio(uint32 dwForIP) const
{
	...
	if (GetDownloadedTotal() < 1048576)	//Changed by thilon on 2007.05.28
		return 1.0F;
	float result = 0.0F;
	if (!GetUploadedTotal())
		result = 10.0F;
...
void CClientCreditsList::LoadList()
{
	...
	try{
		...
		for (UINT i = 0; i < count; i++){
			...
			//ADDED by fengwen on 2006/12/11	<begin> :	格式出错，删除文件。直接退出函数。
			if (newcstruct->nKeySize > MAXPUBKEYSIZE)
			{
				file.Close();
				OnCorruptedFile(file.GetFilePath());
				return;
			}
			//ADDED by fengwen on 2006/12/11	<end> :	格式出错，删除文件。直接退出函数。
...
void CClientCreditsList::OnCorruptedFile(LPCTSTR lpszPathFile)
{
	DeleteFile(lpszPathFile);

	CClientCredits* cur_credit = NULL;
	CCKey tmpkey(0);
	POSITION pos = m_mapClients.GetStartPosition();
