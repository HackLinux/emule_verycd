...
CUpDownClient::~CUpDownClient(){
	...
	else if( theApp.natthread /*&& this peer is NatTrans Peer*/ )			
	{
		...
		theApp.natthread->RemoveNatSocket(GetUserHash());
	}
		...
		switch (temptag.GetNameID())
		{
			...
			case CT_EMULE_UDPPORTS:
				...
				if (temptag.IsInt()) {
					...
					{
						uint16 nUDPPort = theApp.natthread->GetUDPPort(socket);
			...
			default:
				...
				if (thePrefs.GetVerbose())
				{
					...
					AddDebugLogLine(false, _T("********UserName: %s ModString: %s UnTag: %s UserHash: %s IsShareFile: %d"),m_pszUsername,
						m_strModVersion,temptag.GetFullInfo(),userhash,m_fSupportsLargeFiles);
				}
				//-----------------
				//{end}
				//Xman end
...
void CUpDownClient::SendMuleInfoPacket(bool bAnswer){
	...
	if(bSendModVersion) 
	{
		...
		sMyModVersion.Format(_T("VeryCD %u"),VC_VERSION_BUILD);
...
bool CUpDownClient::Disconnected(LPCTSTR pszReason, bool bFromSocket)
{
	...
	if (GetKadState() == KS_QUEUED_FWCHECK_UDP || GetKadState() == KS_CONNECTING_FWCHECK_UDP)
		...
		theApp.clientlist->AddBadBuddy( m_nConnectIP,m_nUserPort,m_nUDPPort,m_achUserHash );
...
bool CUpDownClient::TryToConnect(bool bIgnoreMaxCon, bool bNoCallbacks, CRuntimeClass* pClassSocket)
{
	...
	if ( HasLowID() )	
	{
		if( ACT_FAKETCP==m_nActiveConnectType ) // 9) VeryCD mod support Low2Low Connect...
		{
			...
			if(bLow2LowConn)
			{
				if(socket && theApp.natthread)
				{
					if(theApp.natthread->IsSocketConnected(socket))
					{
						if (CheckHandshakeFinished())
							ConnectionEstablished();
						return true;
	...
	switch (GetUploadState()){
		...
		case US_UPLOADING:
            if(GetPayloadInBuffer() == 0 && GetNumberOfRequestedBlocksInQueue() == 0 && thePrefs.IsExtControlsEnabled()) {
                ...
                if(GetSourceFrom() == SF_LAN)strState = GetResString(IDS_LANTRANSFERRING);// VC-kernel[2007-01-19]:
				else if(socket && socket->m_bUseNat) 
...
bool CUpDownClient::PrepareActiveConnectType()
{
	...
	{		
		if(!theApp.CanDoCallback(this)) /// VeryCD mod support Low2Low Connect
		{
