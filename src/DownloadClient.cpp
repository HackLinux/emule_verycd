...
bool CUpDownClient::AskForDownload()
{
	...
	if (!(socket && socket->IsConnected())) // already connected, skip all the special checks
	{
		...
		if (theApp.listensocket->TooManySockets(GetSourceFrom()==SF_LAN,m_nActiveConnectType==ACT_DIRECTTCP,m_nActiveConnectType==ACT_FAKETCP) 
		...
		if (HasLowID() && !theApp.CanDoCallback(this) && GetLastAskedTime() > 0 && SNT_UNSUPPORT==m_nSupportNatTraverse){
...
void CUpDownClient::SetDownloadState(EDownloadState nNewState, LPCTSTR pszReason){
	if (m_nDownloadState != nNewState){
		...
		if( nNewState==DS_LOWTOLOWIP && m_nSupportNatTraverse==SNT_SUPPORT ) //huby edit
		{
		...
		if (m_nDownloadState == DS_DOWNLOADING ){
			...
			if (socket && nNewState != DS_ERROR && nNewState!= DS_ONQUEUE)
...
//#ifdef _DEBUG_LAN_ // VC-Huby[2006-12-30]: 这个代码逻辑已调试OK
			...
			{
				reaskTime = min( (m_iErrTimes+1)*2*60*1000,FILEREASKTIME); //2Min
			}
			else
