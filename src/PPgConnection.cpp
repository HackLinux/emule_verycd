...
void CPPgConnection::LoadSettings(void)
{
	if (m_hWnd)
	{
		...
		m_ctlMaxUp.SetRange(1, _wtoi(strBuffer)/*thePrefs.GetMaxGraphUploadRate(true)*/);
		...
		m_ctlMaxUp.SetPos((thePrefs.maxupload != UNLIMITED) ? thePrefs.maxupload : _wtoi(strBuffer)/*thePrefs.GetMaxGraphUploadRate(true)*/);
...
void CPPgConnection::OnLimiterChange()
{
	...
	if (maxup < 30 ) 
	{
		if( !IsDlgButtonChecked(IDC_DLIMIT_LBL) )
		{
			::MessageBox(this->m_hWnd, GetResString(IDS_UPLOADTOOLESS), _T("eMule"), MB_OK);
