...
BOOL CPPgWiz1General::OnInitDialog()
{
	...
	GetDlgItem(IDC_USEWEBBROWSER)->ShowWindow(SW_HIDE);
#else
	GetDlgItem(IDC_USEWEBBROWSER)->SetWindowText(GetResString(IDS_PW_WEBBROWSER)); // added by GGSoSo for webbrowser
#endif
	//Chocobo End
	return TRUE;
}
...
LRESULT CPPgWiz1Dir::OnWizardNext()
{
	...
	{
		strTempDir = thePrefs.GetMuleDirectory(EMULE_TEMPDIR);
