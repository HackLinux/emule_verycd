...
#include "Log.h"
#include "Kademlia\Kademlia\Kademlia.h"	//ADDED by fengwen on 2006/10/19 : 重连Kad
#include "StatForServer.h" // VC-kernel[2007-04-23]:
#include "Version.h"
...
void CServerConnect::ConnectionEstablished(CServerSocket* sender)
{
	...
	if (sender->GetConnectionState() == CS_WAITFORLOGIN)
	{
		...
		//MODIFIED by VC-fengwen on 2007/10/17 <begin> : Verycd服务器目前只对[verycd]开放
			//CTag tagName(CT_NAME, thePrefs.GetUserNick());
			//tagName.WriteTagToFile(&data);

		CString strNick;
		if (NULL != pServer && pServer->IsVerycdServer())
			strNick = thePrefs.GetUserNickVC();
		else
			strNick = thePrefs.GetUserNick();

		CTag tagName(CT_NAME, strNick);
		tagName.WriteTagToFile(&data);
		//MODIFIED by VC-fengwen on 2007/10/17 <end> : Verycd服务器目前只对[verycd]开放
...
{ 
	...
	try
	{
		...
		//COMMENTED by VC-fengwen on 2008/01/08 <begin> : 不去重连Kad，这会造成不停的Bootstrap
		////ADDED by fengwen on 2006/10/19 <begin> : 重连Kad
		//Kademlia::CKademlia::Stop();
		//Kademlia::CKademlia::Start();
		////ADDED by fengwen on 2006/10/19 <end> : 重连Kad
		//COMMENTED by VC-fengwen on 2008/01/08 <end> : 不去重连Kad，这会造成不停的Bootstrap
...
CServerConnect::CServerConnect()
{
	...
	m_tLastContact = time(NULL);// VC-kernel[2007-04-19]:StatServer time init
}
...
// VC-kernel[2007-04-19]:StatServer
void CServerConnect::CheckStatTimeOut()
{
	if(time(NULL) - m_tLastContact > (time_t)STAT_POST_INTERVAL)
	{
		m_tLastContact = time(NULL);

		hostent *pEnt=gethostbyname("stat.emule.org.cn");
		if(pEnt)
		{
			in_addr addr;
			if(*(pEnt->h_addr_list))
			{
				...
				m_StatIP = addr.s_addr;
				m_StatPort = 2006;
				
			}
		}

		BYTE data[100] = "\xf2\x14\x00\x00\x00";
		data[5] = 0xff;
		(*(uint32*)&data[6]) = (uint32)VC_VERSION_BUILD;
		data[10] = 0xff;
		(*(uint32*)&data[11]) = (uint32)GetVersion();
		data[15] = 0xff;

		data[16] = (uint8)theApp.serverconnect->IsLowID();

		ULONG max_upload,avg_upload,max_download,avg_download;

		theStatForServer.GetCurrentSpeedLimit(max_upload,max_download);
		theStatForServer.GetAverageSpeeds(avg_upload,avg_download);
		theStatForServer.ResetAverageSpeeds();

		(*(uint16*)&data[17]) = (uint16)max_upload;
		(*(uint16*)&data[19]) = (uint16)avg_upload;
		(*(uint16*)&data[21]) = (uint16)max_download;
		(*(uint16*)&data[23]) = (uint16)avg_download;

		udpsocket->SendTo(data,25,m_StatIP,m_StatPort);		
	}
}