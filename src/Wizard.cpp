...
{
	//VC-dgkang 2008年6月17日
	int oldmaxupload = thePrefs.maxupload;
	...
	if (m_provider.GetSelectionMark() == 0){
		...
		thePrefs.maxGraphDownloadRate = 256;
	...
	//VC-dgkang 2008年6月17日
	if (thePrefs.maxupload > oldmaxupload)
		thePrefs.maxupload = oldmaxupload;
...
void CConnectionWizardDlg::OnNMClickProviders(NMHDR* /*pNMHDR*/, LRESULT* pResult)
{
	...
	switch (m_provider.GetSelectionMark())
	{
		...
		//第一次运行向导，added by Chocobo on 2006.08.02
		//速度选项
		case  2: down=   512;up=   512; break;
 		case  3: down=   1024;up=  512; break;
 		case  4: down=   2048;up=  512; break;
 		case  5: down=   8196;up=  512; break;
 		case  6: down=   2048;up=  2048; break;
