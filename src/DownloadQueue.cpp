...
void CDownloadQueue::AddFileLinkToDownload(CED2KFileLink* pLink, int cat)
{
	...
	{
		if(thePrefs.m_bUpdate)
		{
			thePrefs.m_UpdateHash = md4str(newfile->GetFileHash());
			CIni ini(thePrefs.GetConfigFile(), L"update");
			ini.WriteString( L"Updatehash",thePrefs.m_UpdateHash );	//
		}
...
void CDownloadQueue::AddToResolved( CPartFile* pFile, SUnresolvedHostname* pUH, LPCTSTR lpszRefer /*= NULL*/)
{
	if( pFile && pUH )
		m_srcwnd.AddToResolve( pFile->GetFileHash(), pUH->strHostname, pUH->nPort, pUH->strURL, lpszRefer);
}
...
void CDownloadQueue::AddDownload(CPartFile* newfile,bool paused) {
	...
	ExportPartMetFilesOverview();
	//ADDED by fengwen on 2006/09/29 <begin> : Get url source from server
	if (!newfile->m_bAlreadyFetchUrlSrc)
		newfile->m_urlSrcFromSvrMgr.GetSrcFromServerAsyn();
	//ADDED by fengwen on 2006/09/29 <end> : Get url source from server
#ifdef _ENABLE_LAN_TRANSFER
    theApp.internalsocket -> Broadcast(newfile -> GetFileHash());   // VC-kernel[2007-01-11]:
#endif
}
...
/*
    ...
    if(IsLanIP(source->GetIP()) )
    {
        source->SetSourceFrom(SF_LAN);
    }
...
*/
	...
	theApp.emuledlg->transferwnd->downloadlistctrl.AddSource(sender,source,false); //UI
	return true;
}
...
bool CDownloadQueue::CheckAndAddKnownSource(CPartFile* sender,CUpDownClient* source, bool bIgnoreGlobDeadList){
    ...
    uint32 nClientIP = ntohl(source->GetUserIDHybrid());// VC-kernel[2007-01-18]:moved here
	if (!source->HasLowID()){
...
void CSourceHostnameResolveWnd::AddToResolve(const uchar* fileid, LPCSTR pszHostname, uint16 port, LPCTSTR pszURL, LPCTSTR lpszRefer/* = NULL*/)
{
	...
	entry->port = port;
	entry->strURL = pszURL;
	entry->strRefer = lpszRefer;		//ADDED by fengwen on 2006/09/29 : 传递Refer参数，以在请求url时填写。{22555E64-6826-4f57-B027-49617DA784F6}
			...
			if (pHost->h_length == 4 && pHost->h_addr_list && pHost->h_addr_list[0])
			{
				...
				if (file)
				{
					...
					else
					{
						...
						file->AddSource(resolved->strURL, nIP, resolved->strRefer);
...
// Added by Soar Chin 10/17/2007 to support multiurl parameter
void CDownloadQueue::AddUrlListToDownload( CString &url, int cat /*= 0*/ )
{
	LPCTSTR urls = url;
	CString aurl;
	INT len = _tcslen(urls), i = 0, j = 0;
	for(i = 0, j = _tcscspn(urls + i, _T("\r\n")); i < len; i += j + 1, j = _tcscspn(urls + i, _T("\r\n")))
	{
		if(j > 10)
		{
			aurl.SetString(urls + i, j);
			if(TestUrl(aurl))
			{
				...
				if(pLink->GetKind() == CED2KLink::kFile)
				{
					...
					if(pFileLink)
						theApp.downloadqueue->AddFileLinkToDownload(pFileLink,cat);
				}
				delete pLink;
			}
		}
