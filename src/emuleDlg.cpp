...
BOOL CemuleDlg::OnCommand(WPARAM wParam, LPARAM lParam)
{
	...
	{	
		...
		case TBBTN_HELP:
			//Chocobo Start
			//在内置浏览器打开帮助链接，modified by Chocobo
			if(webbrowser!=NULL
...
void CemuleDlg::HtmlHelp(DWORD_PTR dwData, UINT nCmd)
{
	...
	if (bHelpError)
	{
		...
		{
			//Chocobo Start
			//eMule帮助链接，modified by Chocobo on 2006.08.01
	...
	case 1:			//TCP
		{
			...
			{
				if ( SUCCEEDED(hr) || theApp.serverconnect->IsLowID() )		//ADDED by VC-fengwen on 2007/09/04 : 如果失败了，但已经是高Id了则不提示。
				{
					strErrorMessage = CUPnpMgr::Result2String(hr, pResult->dwActionErrorCode);
					strLogText.Format(_T("UPnp: TCP 端口映射 %d-->%d (%s)"),
	...
	case 2:			//UDP
		{
			...
			{
				strErrorMessage = CUPnpMgr::Result2String(hr, pResult->dwActionErrorCode);
				strLogText.Format(_T("UPnp: UDP 端口映射 %d-->%d (%s)"),
									pResult->wInternalPort,
									pResult->wExternalPort,
									strErrorMessage);
