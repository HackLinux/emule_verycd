...
#include "PlayerMgr.h" // VC-SearchDream[2007-05-22]: for see movie while downloading
    ...
    try
    {
        ...
        for (UINT j = 0; j < tagcount; j++)
        {
			...
			if (pOutCheckFileFormat == NULL || (pOutCheckFileFormat != NULL && (newtag->GetNameID()==FT_FILESIZE || newtag->GetNameID()==FT_FILENAME))){
                ...
                {
                    ASSERT(newtag->IsStr());
                    RecordUrlSource(newtag->GetStr());		//	just load them into m_strlstUrlSources,
...
uint32 CPartFile::Process(uint32 reducedownload, UINT icounter/*in percent*/)
{
	...
	if (theApp.emuledlg->IsRunning()) // may be called during shutdown!
	{
		...
		{
			theApp.emuledlg->ShowPopMule(this);			
    ...
    else
    {
        ...
        for (POSITION pos = srclist.GetHeadPosition(); pos != NULL;)
        {
            ...
            case DS_LOWTOLOWIP:
            {
                ...
                if ( cur_src->HasLowID() )
                {
                    ...
                    if ( !theApp.CanDoCallback( cur_src ) && !cur_src->IsSupportTraverse() )
                    {
            ...
            case DS_WAITCALLBACKKAD:
            {
                ...
                if (cur_src->GetSourceFrom() == SF_LAN)cur_src->AskForDownload();// VC-kernel[2007-01-18]:
                break;
            }
            }
        }
        ...
        if ( NeedMoreSourceFromKad() )
        {
            ...
            //if (theApp.downloadqueue->DoKademliaFileRequest() && (Kademlia::CKademlia::GetTotalFile() < KADEMLIATOTALFILE) && (dwCurTick > m_LastSearchTimeKad) &&  Kademlia::CKademlia::IsConnected() && theApp.IsConnected() && !stopped)
			if (theApp.downloadqueue->DoKademliaFileRequest() && (Kademlia::CKademlia::GetTotalFile() < KADEMLIATOTALFILE) && (dwCurTick > m_LastSearchTimeKad) &&  Kademlia::CKademlia::IsConnected() && theApp.IsConnected() && !stopped && Kademlia::CKademlia::IsReady())
    ...
    {
        theApp.internalsocket -> Broadcast(GetFileHash());
    }
#else
    if (time(NULL)%(60*10) == 0)
    {
...
bool CPartFile::CanAddSource(uint32 userid, uint16 port, uint32 serverip, uint16 serverport, UINT* pdebug_lowiddropped, bool Ed2kID)
{
    ...
    if ( IsLowID(hybridID) && theApp.IsFirewalled())
    {
        if (pdebug_lowiddropped)
            (*pdebug_lowiddropped)++;
        if ( port!=uint16(-1) ) // VC-Huby[2007-01-12]: -1 是支持NatTrans的lowid
            return false;
    }
    // MOD Note - end
    return true;
...
void CPartFile::PerformFileCompleteEnd(DWORD dwResult)
{
    if (dwResult & FILE_COMPLETION_THREAD_SUCCESS)
    {
		...
		if ( ( GetFileType() == L"Audio" || GetFileType() == L"Video" ) && !IsSafeFile( GetFullName() ) )
		{
			//不安全的文件
			...
			theApp.emuledlg->ShowNotifier( GetResString(IDS_TBN_DOWNLOADBADFILE) + _T('\n') + GetFileName() + L"."  + GetResString(IDS_UNSAFE), TBN_DOWNLOADFINISHED, GetFilePath() );
		}
		else
		{
...
void CPartFile::ResumeFile(bool resort)
{
    ...
    ReAddAllUrlSrcToDownloadQueue();	//ADDED by fengwen on 2006/09/01 : add recorded url sources when resume file.

#ifdef _ENABLE_LAN_TRANSFER
    theApp.internalsocket -> Broadcast(GetFileHash());  // VC-kernel[2007-01-11]:
#endif
...
void CPartFile::RecordUrlSource(LPCTSTR lpszUrl)
{
    if (IsExistInUrlSourcesList(lpszUrl))
        return;
    ...
    pos = m_strlstUrlSources.GetHeadPosition();
    while (NULL != pos)
    {
        str = m_strlstUrlSources.GetAt(pos);

        ParseUrlSourceRecord(str, strUrl, strRefer);
        if (0 == strUrl.CompareNoCase(strAddUrl))
        {
            m_strlstUrlSources.SetAt(pos, lpszUrl);
            return;
        }

        m_strlstUrlSources.GetNext(pos);
    }


    m_strlstUrlSources.AddTail(lpszUrl);
}

BOOL CPartFile::IsExistInUrlSourcesList(LPCTSTR lpszUrl)
{
    POSITION	pos;
    CString		str;
    ...
    while (NULL != pos)
    {
        str = m_strlstUrlSources.GetNext(pos);
...
void CPartFile::AddUrlSrcToDownloadQueue(LPCTSTR lpszUrl)
{
    ...
    theApp.downloadqueue->AddToResolved(this, &uhn, strRefer);
}
...
void CPartFile::OnUrlSrcFromSvrFetched(const CStringList *pUrlList)
{
    ...
    while (NULL != pos)
    {
        ...
        if (!IsExistInUrlSourcesList(str))
        {
            RecordUrlSource(str);
            AddUrlSrcToDownloadQueue(str);
        }
...
{
    if ( !theApp.serverconnect->IsLowID() ) //HighId
    {
...
void CPartFile::SetSeeOnDownloading(bool bSee)
{
	m_bSeeOnDownloading = bSee;
...
{
	return IsComplete(0, 1024*1024, true);
}
...
//Write by jimmyc
BOOL CPartFile::IsSafeFile( const CString &strFileFullName )
{
