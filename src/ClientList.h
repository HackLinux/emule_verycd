...
{
public:
	CBadBuddy( const DWORD dwIP,uint16 iPort,uint16 iUdpPort ):m_dwIP(dwIP),m_iPort(iPort),m_iUdpPort(iUdpPort),m_iLostTimes(0),m_iLastConnect(0) {};
	...
	uint16 m_iLostTimes;
	uint32 m_iLastConnect;
};
typedef CTypedPtrList<CPtrList, CBadBuddy*> CBadBuddyList;
//typedef CList<CBadBuddy> CBadBuddyList;
...
public:	
	CBadBuddy* AddBadBuddy( DWORD dwIP,uint16 iTcpPort,uint16 iUdpPort,const uchar* pUserHash );
	bool IsBadBuddy( DWORD dwIP,uint16 iTcpPort );
	CBadBuddy* GetCandidateBuddyFromBlackList( ); 
private:	
	CBadBuddyList m_BadBuddyList;// VC-Huby[2006-12-19]:Bad Buddy Blacklist
};
