...
#include "Log.h"
#include "Sockets.h"
#include "UPnpAsynThreads.h"	//ADDED by fengwen on 2006/11/02 : 


//#include "FirewallOpener.h" // Added by thilon on 2006.10.19 for  IFWS - [ICSFirewall]
...
CUDPSocket::CUDPSocket()
{
	...
	m_port = 0; //Added by thilon on 2006.10.17 [BindToAdapter]
}
...
bool CUDPSocket::Rebind()
{
	...
	if(!Create()){
		LogError(LOG_STATUSBAR, GetResString(IDS_MAIN_SOCKETERROR),thePrefs.GetServerUDPPort());
		return false;
	}
...
//		if (!CAsyncSocket::Create(thePrefs.GetServerUDPPort()==0xFFFF ? 0 : thePrefs.GetServerUDPPort(), SOCK_DGRAM, FD_READ | FD_WRITE, theApp.GetBindAddress()/*thePrefs.GetBindAddrW()*/)){//Changed by thilon on 2006.10.19
...
//			bool bResult = (theApp.m_pFirewallOpener->OpenPort(m_port, NAT_PROTOCOL_UDP, EMULE_DEFAULTRULENAME_SVR, thePrefs.GetICFClearOnClose() || thePrefs.GetServerUDPPort()==0xFFFF));
//			Log(GetResString(bResult ? IDS_FO_TEMPUDP_S : IDS_FO_TEMPUDP_F), m_port);
//		}*/
//
//		if(mapping)
//		{
...
//			mapping->ref = &mapping;
//
//			mapping->internalPort = mapping->externalPort = m_port/*(WORD)port*/;
//			mapping->protocol = CUPnP::UNAT_UDP;
...
//COMMENTED by fengwen on 2006/11/03 <begin>	: 不去打开Server的udp端口，因为Server的udp是随机产生，较容易出现异常而滞留在upnp表项中。如果Server UDP端口有重要功能时，再打开这里的注释。
		////ADDED by fengwen on 2006/11/02 <begin> : 打开UPnp端口映射。
		//if (thePrefs.GetUPnPNat())
		//{
		//	CString client;
		//	UINT port;
		...
		//	mapping.m_wExternalPort = (WORD) port;
		//	mapping.m_wInternalPort = mapping.m_wExternalPort;
		//	mapping.m_strDescription = strDescription + _T(" Server UDP");

		//	CUPnpAsynThreads::AddNatPortMappingAsyn(&(theApp.m_upnpMgr),
		//		mapping,
		//		NULL,			//由于此时emuleDlg还没有创建。所以就不发消息通知了。默默的加上就行了。
		//		WM_NULL,
		//		thePrefs.GetUPnPNatTryRandom());
		//}
		////ADDED by fengwen on 2006/11/02 <end> : 打开UPnp端口映射。
//COMMENTED by fengwen on 2006/11/03 <end>		: 不去打开Server的udp端口，因为Server的udp是随机产生，较容易出现异常而滞留在upnp表项中。如果Server UDP端口有重要功能时，再打开这里的注释。
