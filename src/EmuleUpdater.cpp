...
{
	m_iMajor	= CemuleApp::m_nVersionMjr;
	m_iMinor	= CemuleApp::m_nVersionMin;
	m_iUpdate	= CemuleApp::m_nVersionUpd;
	m_iBuild	= CemuleApp::m_nVCVersionBld;
}
...
void CEmuleUpdater::Check(const CString& strURL)
{
	...
	HINTERNET hUrl = InternetOpenUrl(hInet, strURL, NULL, -1L,INTERNET_FLAG_RELOAD | INTERNET_FLAG_PRAGMA_NOCACHE | INTERNET_FLAG_NO_CACHE_WRITE |WININET_API_FLAG_ASYNC, NULL);
...
//		SetProgressRange(dwFileSize);
	...
	//DWORD dwStartTicks = ::GetTickCount();// VC-linhai[2007-08-06]:局部变量已初始化但不引用
	//DWORD dwCurrentTicks = dwStartTicks;	// VC-linhai[2007-08-06]:局部变量已初始化但不引用
	DWORD dwBytesRead = 0;
	char szReadBuf[1024];
	DWORD dwBytesToRead = 1024;
	//DWORD dwTotalBytesRead = 0;
	//DWORD dwLastTotalBytes = 0;			// VC-linhai[2007-08-06]:局部变量已初始化但不引用
	//DWORD dwLastPercentage = 0;			// VC-linhai[2007-08-06]:局部变量已初始化但不引用
