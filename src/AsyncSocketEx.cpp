		...
		try
		{
			...
			if (message >= WM_SOCKETEX_NOTIFY)
			{
				...
				if (message < (UINT)(WM_SOCKETEX_NOTIFY + pWnd->m_nWindowDataSize)) //Index is within socket storage
				{
					...
					if (!pSocket)
						return 0;
					if (hSocket == INVALID_SOCKET && !pSocket->m_bUseNat)
						return 0;
					...
					if (!pSocket->m_pFirstLayer)
					{
						...
						switch (nEvent)
						{
							...
							case FD_CLOSE:
								...
								break;
							case FD_FAILED:
								pSocket->OnFailConnect(nErrorCode);
								break;
						}
					}
	#ifndef NOLAYERS
			...
			else if (message == WM_SOCKETEX_GETHOST)
			{
				...
				LPIN_ADDR pin_addr = (LPIN_ADDR)((LPHOSTENT)pSocket->m_pAsyncGetHostByNameBuffer)->h_addr;
				if(NULL==pin_addr)
				{
					ASSERT(0);
					return 0;
				}
				sockAddr.sin_addr.s_addr = pin_addr->s_addr;
...
CAsyncSocketEx::CAsyncSocketEx()
{
	...
	m_bUseNat = false;
	m_pTraverseFac = NULL; ///new CTraverseBySvrFac;
...
CAsyncSocketEx::~CAsyncSocketEx()
{
	//  added by yunchenn, 2006/12/04
	if( m_bUseNat && theApp.natthread )
	{
		theApp.natthread->RemoveSocket(this);
	}
							...
							LPCSTR lpszSocketAddress /*=NULL*/, BOOL bReuseAddr /*=FALSE*/ )
{
...
#endif //NOLAYERS
	{
		if(m_bUseNat)
		{
			ASSERT(m_SocketData.hSocket==INVALID_SOCKET);
		}
		else
		{
...
int CAsyncSocketEx::Receive(void* lpBuf, int nBufLen, int nFlags /*=0*/)
{
	...
	if(m_bUseNat)
	{
		return theApp.natthread?theApp.natthread->Receive(this, lpBuf, nBufLen) : -1;
		//return -1;
	}
#ifndef NOLAYERS
	if (m_pFirstLayer)
...
int CAsyncSocketEx::Send(const void* lpBuf, int nBufLen, int nFlags /*=0*/)
{
	...
	if(m_bUseNat)
	{
		return theApp.natthread?theApp.natthread->Send(this, lpBuf, nBufLen):-1;
		//return -1;
	}
#ifndef NOLAYERS
	if (m_pFirstLayer)
...
BOOL CAsyncSocketEx::Connect(const SOCKADDR* lpSockAddr, int nSockAddrLen)
{
	...
	if(m_bUseNat)
	{
		//if(theApp.natthread->IsSocketConnected(this))
		//{
		//	CClientReqSocket * pClientSock = DYNAMIC_DOWNCAST(CClientReqSocket, this);
		...
		CUpDownClient * pClient = theApp.clientlist->FindClientByUserHash((uchar*)lpSockAddr);
		...
		if (pClient)
		{
			...
			if (pClient->GetSourceFrom() == SF_SOURCE_EXCHANGE)
			{
				m_pTraverseFac = new CTraverseBySEFac();
			}
			else
			{
				m_pTraverseFac = new CTraverseBySvrFac();
			}
		}

		//if(theApp.natthread) theApp.natthread->Connect(this, (uchar*)lpSockAddr);
		if(theApp.natthread && m_pTraverseFac) 
		{
			theApp.natthread->Connect(this, (uchar*)lpSockAddr, m_pTraverseFac);
			return TRUE;
		}
		return FALSE;
	}
	else // VC-SearchDream[2007-06-28]: Add 'else' scope and move the following code to it
	{	
		//  added by yunchenn
#ifndef NOLAYERS
	if (m_pFirstLayer)
...
{
	if( m_bUseNat )
		return TRUE; // [11/26/2007 huby]: FakeTcpSocke不需要靠Win-OS的网络内核发 Event通知,call了WSAAsyncSelect也是无效Call
...
{
	if (m_SocketData.hSocket == INVALID_SOCKET && !m_bUseNat)
		return FALSE;
	...
	if(m_bUseNat)
	{
		if(!m_pLocalAsyncSocketExThreadData)
		{
			InitAsyncSocketExInstance();
		}
		if(m_SocketData.nSocketIndex == -1)
		{
			AttachHandle(NULL);
		}
